...
proc add_implementation { name } {
   ...
   set_impl_attribute $name "phys"                0
...
proc add_ooc_implementation { name } {
   ...
   set_ooc_attribute $name "budget.create"       1
...
###############################################################
proc set_impl_attribute { impl attribute value } {
...
###############################################################
proc set_config_attribute { config attribute value } {
...
###############################################################
proc set_ooc_attribute { impl attribute value } {
   global oocAttribute
   set procname [lindex [info level 0] 0]
...
###############################################################
proc set_module_attribute { module attribute value } {
...
proc sort_configurations { } {
   ...
   foreach config $configs {
      ...
      foreach setting $settings {
         lassign $setting name module state
         if {[string match $module $top]} {
            puts "\t${config}\t(State of Static: $state\tRun Implementation: $impl_enum)"
         }
      }
   }
   puts "\n"
