   ...
   if {[file exists $prj]} {
      ...
      foreach line $prj_lines {
         ...
         if {[llength $line] > 0 && ![string match "#*" $line]} {
            ...
            if {![string match $type "header"]  && \
                ![string match $type "system"]  && \
                ![string match $type "verilog"] && \
                ![string match $type "vhdl"]} {
            ...
            if {[file exists ${srcDir}/$file]} {
               ...
               if {[string match $type "vhdl"]} {
                  command "set_property LIBRARY $lib \[get_files $file\]"
               }
               if {[string match $type "system"]} {
