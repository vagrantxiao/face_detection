   ...
   if { [catch {current_instance > $resultDir/temp.log}] } {
      ...
      if {[info exists checkpoint1] || [info exists checkpoint2]} {
         if {[file exists $checkpoint1]} {
            puts "\tReading checkpoint $checkpoint1 for $instance"
            command "read_checkpoint $checkpoint1" "$resultDir/read_checkpoint_${instance}_$phase.log"
            if { [catch {current_instance > $resultDir/temp.log}] } {
               command "link_design"
            }
         } elseif {[file exists $checkpoint2]} {
            puts "\tReading checkpoint $checkpoint2 for $instance"
            command "read_checkpoint $checkpoint2" "$resultDir/read_checkpoint_${instance}_$phase.log"
            if { [catch {current_instance > $resultDir/temp.log}] } {
               command "link_design"
            }
         } else {
