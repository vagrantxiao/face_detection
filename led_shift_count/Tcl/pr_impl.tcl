...
proc pr_impl { impl } {
   ...
   set bitstream           [get_config_attribute $impl bitstream]
   set bitstream.pre       [get_config_attribute $impl bitstream.pre]
   set bitstream_options   [get_config_attribute $impl bitstream_options]
   ...
   if {$link} {
      ...
      if {[string match $staticState "import"]} {
         set time [clock seconds]
         puts "\tLocking $top \[[clock format $time -format {%a %b %d %H:%M:%S %Y}]\]"
   ...
   if {$bitstream} {
      impl_step write_bitstream $top $bitstream_options none ${bitstream.pre}
   }
