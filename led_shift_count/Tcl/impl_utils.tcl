...
proc get_module_file { module } {
   ...
   if {[file exists $synthDCP]} {
      set moduleFile $synthDCP
   } elseif {[file exists $synthDir/$module/${moduleName}_synth.dcp]} {
      set moduleFile $synthDir/$module/${moduleName}_synth.dcp
   } elseif {[file exists $netlistDir/$module/${moduleName}.edf]} {
      set moduleFile $netlistDir/$module/${moduleName}.edf
   } elseif {[file exists $netlistDir/$module/${moduleName}.edn]} {
      set moduleFile $netlistDir/$module/${moduleName}.edn
   } elseif {[file exists $netlistDir/$module/${moduleName}.ngc]} {
      set moduleFile $netlistDir/$module/${moduleName}.ngc
   } else {
      ...
      foreach setting $settings {
         ...
         if {[string match $cell $top]} {
            ...
            if {[file exists $configFile]} {
               command "read_checkpoint $configFile" "$bitDir/read_checkpoint_$config.log"
...
proc verify_configs { configs } {
   ...
   for {set i 1} {$i < $numConfigs} {incr i} {
      ...
      puts $rfh "\tRunning pr_verify on $initialConfig and $config"
      puts "\tRunning pr_verify on $initialConfig and $config"
      set logFile "pr_verify_${initialConfig}_${config}.log"
      command "pr_verify -full_check $initialConfigFile $configFile" $logFile
      ...
      if {[file exists $logFile]} {
         ...
         puts $rfh "ERROR: Could not find specified log file \"$log\"."
